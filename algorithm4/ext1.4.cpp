...
using namespace std;
/*
 * If an integer is randomly chosen from the range 1~N,
 * how many times we need to choose, before the first time 
 * a repeated integer is chosen? It is said that the expected
 * number of chooses is about C*sqrt(N), where C is a constant.
 * This program validate this hypothesis.
 * This program could also answers the expected number of people we
...
/*
 * This program validates the hypothesis that the number of integers generated
 ...
 * How many balls are expected to thrown before every bin contains at least one ball?
 *
 * This program answers the above problem.
 */
...
void generateBitonicArray(int max, int min, int n, vector<int>& result) {
	...
	set<int, std::greater<int> >::iterator start = rHalf.begin(), term =
			rHalf.end();
	while (start != term) {
...
/*
 * a bitonic array is an array compromised of an increasing sequence of
...
int testBitonicSearch(int argc, char *argv[]) {
	...
	for (int i = 0; i < iteration2; ++i) {
		...
		for (int j = 0; j < iteration1; ++j) {
			int key = rand();
			bool ret1 = (bitonicSearch(&numbers[0], numbers.size(), key,
					numOfComparison) != -1);
			bool ret2 = (std::find(numbers.begin(), numbers.end(), key)
					!= numbers.end());
			assert(ret1 == ret2);
		}
		cout << "expected : " << 3 << ", actual : "
				<< (numOfComparison / (double) iteration1) / expected << endl;
	}
}
/*
 ...
 */
template<class T>
inline bool isLocalMinimum(const T *array2D, unsigned int m, unsigned int n,
		unsigned int i, unsigned int j, unsigned int& comparisions) {
...
template<class T>
inline bool isLocalMinimumFast(const T *array2D, unsigned int n, unsigned int i,
		unsigned int j, unsigned int& comparisions) {
	unsigned int index = i * n + j;
	comparisions += 1;
	if (array2D[index] > array2D[index - n])
		return false;
...
template<class T>
void output(ostream& os, const T *vals, unsigned int n, unsigned int startRow,
		unsigned int endRow, unsigned int startCol, unsigned int endCol) {
...
void generateSpecialArray(int *output, unsigned int m, unsigned int n,
		unsigned int min, unsigned int row, unsigned int col) {
	if (row >= m)
		row = m - 1;
	if (col >= n)
		col = n - 1;
	...
	if (row % 2 != 0) {
		...
		cout << "offset " << offset << ", total " << total << ", row " << row
				<< ", col " << col << endl;
...
template<class T>
pair<unsigned int, unsigned int> buggyLocalMinimum(const T * array2D,
		unsigned int m, unsigned int n, unsigned int& comparisions) {
	...
	while (startRow != endRow && startCol != endCol) {
		...
		if (array2D[midRow * n + colMin] < array2D[rowMin * n + midCol]) {
			...
			if (array2D[midRow * n + colMin]
					> array2D[(midRow + 1) * n + colMin]) {
				startRow = midRow + 1;
			...
			//We could guarantee that midCol + 1 <= endCol,
			//that is we won't access memory out of the 2d array
			if (array2D[rowMin * n + midCol]
					> array2D[rowMin * n + midCol + 1]) {
				startCol = midCol + 1;
...
template<class T>
pair<unsigned int, unsigned int> localMinimum(const T * array2D, unsigned int m,
		unsigned int n, unsigned int& comparisions) {
	typedef pair<unsigned int, unsigned int> Type;
	if (m == 0 || n == 0)
		return Type(-1, -1);
	if (m == 1)
	...
	unsigned int tmpRow = 0, tmpCol = 0, tmpIndex = 0;
	/*
	 * check elements in the four corner of the 2D array
	 */
	// Check the top left corner.
	if (array2D[0] <= array2D[1] && array2D[0] <= array2D[n])
		return Type(0, 0);

	// Check the top right corner.
	tmpIndex = n - 1;
	if (array2D[tmpIndex] <= array2D[tmpIndex - 1]
			&& array2D[tmpIndex] <= array2D[tmpIndex + n])
		return Type(0, n - 1);
	...
	/*
	 * Check the element on the four borders of the array.
	 * And find the minimum.
	 */
	// Check he element on the top border of the 2D array.
	for (unsigned int i = 1; i < n - 1; ++i) {
		if (array2D[i] <= array2D[i - 1] && array2D[i] <= array2D[i + 1]
				&& array2D[i] <= array2D[i + n])
			return Type(0, i);
		if (array2D[i] < array2D[minRow * n + minCol]) {
			minRow = 0;
			minCol = i;
		}
	}

	// Check the element on the bottom border of the 2D array.
	tmpIndex = (m - 1) * n + n - 1;
	for (unsigned int i = (m - 1) * n + 1; i < tmpIndex; ++i) {
		if (array2D[i] <= array2D[i - 1] && array2D[i] <= array2D[i + 1]
				&& array2D[i] <= array2D[i - n])
			return Type(m - 1, i % n);
		if (array2D[i] < array2D[minRow * n + minCol]) {
	...
	// Check he element on the left border of the 2D array.
	for (unsigned int i = 1; i < m - 1; ++i) {
		unsigned int tmpIndex = i * n;
		if (array2D[tmpIndex] <= array2D[tmpIndex - n]
				&& array2D[tmpIndex] <= array2D[tmpIndex + 1]
				&& array2D[tmpIndex] <= array2D[tmpIndex + n])
			return Type(i, 0);
		if (array2D[i] < array2D[minRow * n + minCol]) {
			minRow = i;
	...
	// Check he element on the right border of the 2D array.
	for (unsigned int i = 1; i < m - 1; ++i) {
		unsigned int tmpIndex = i * n + n - 1;
		if (array2D[tmpIndex] <= array2D[tmpIndex - n]
				&& array2D[tmpIndex] <= array2D[tmpIndex - 1]
				&& array2D[tmpIndex] <= array2D[tmpIndex + n])
			return Type(i, 0);
	...
	//If we reach here, then the local minimum is not on the borders of the array.
	unsigned int startRow = 0, endRow = m - 1;
	unsigned int startCol = 0, endCol = n - 1;
	while (startRow < endRow - 1 && startCol < endCol - 1) {
		...
		for (unsigned int i = startCol + 1; i <= endCol - 1; ++i) {
			if (isLocalMinimumFast(array2D, n, midRow, i, comparisions))
				return Type(midRow, i);
			if (array2D[midRow * n + i] < array2D[minRow * n + minCol]) {
				minRow = midRow;
				minCol = i;
			}
			++comparisions;
		...
		for (unsigned int i = startRow + 1; i <= endRow - 1; ++i) {
			if (isLocalMinimumFast(array2D, n, i, midCol, comparisions))
				return Type(i, midCol);
			if (array2D[i * n + midCol] < array2D[minRow * n + minCol]) {
				minRow = i;
				minCol = midCol;
			}
			++comparisions;
		}
		//Decide the next sub area to search.
		comparisions += 3;
		if (minRow != midRow) {
			//The minimum value is not in the middle row.
			if (minRow < midRow) {
				//The top half
				endRow = midRow;
			} else {
			...
			if (minCol < midCol) {
				//the left half
				endCol = midCol;
			} else if (minCol > midCol) {
				//the right half
				startCol = midCol;
			} else {
				/*
				 * If the minimum element is larger than the element to the right of it,
				 * then we could guarantee a local minimum at the right half,
				 * else we could guarantee a local minimum at the left half.
				 */
				if (array2D[minRow * n + minCol]
						> array2D[(minRow + 1) * n + minCol + 1]) {
					// the right half
					startCol = midCol;
				} else {
					// the left half
			...
			//The minimum value is in the middle row.
			if (minCol < midCol) {
				// the left half
				endCol = midCol;
			} else {
			...
			/*
			 * If the minimum element is larger than the element below it,
			 * then we could guarantee a local minimum at the bottom half,
			 * else we could guarantee a local minimum at the top half.
			 */
			if (array2D[minRow * n + minCol]
					> array2D[(minRow + 1) * n + minCol]) {
				// the bottom half
				startRow = midRow;
			} else {
				// the top half
	...
	/*if (startRow == endRow) {
	 for (unsigned int j = startCol; j <= endCol; ++j)
	 if (isLocalMinimumFast(array2D, n, startRow, j, comparisions))
	 return Type(startRow, j);
	 //ofstream out("test.txt");
	 output(cout, array2D, m, n);
	 cout << "startRow = " << startRow << ", startCol = " << startCol
	 ...
	 if (startCol == endCol) {
	 for (unsigned int j = startRow; j <= endRow; ++j)
	 if (isLocalMinimumFast(array2D, n, j, startCol, comparisions))
	 return Type(j, startCol);
	 output(cout, array2D, m, n);
	 cout << "startRow = " << startRow << ", startCol = " << startCol
	 << ", endCol = " << endCol << endl;
...
void testLocalMinimun(int argc, char *argv[]) {
	...
	//test for local minimum of 2D array
	{
		...
		localMinimum(failArray, row, col, comparisions);
		cout << "local minimum is at (" << index.first << "," << index.second
				<< ") : " << failArray[index.first * col+ index.second] << endl;
	}
	{
		...
		for (int j = 0; j < 1; ++j) {
			...
			comparisions = 0;
			pair<unsigned int, unsigned int> index = /*buggyLocalMinimum(vals, 7,
			 9, comparisions);*/localMinimum(vals, 7, 9, comparisions);
			/*if (comparisions == 0)
			 for (unsigned int i = 0; i < m; ++i) {
			 for (unsigned int j = 0; j < n; ++j) {
			...
			cout << "local minimum is at (" << index.first << ","
					<< index.second << ") : "
					<< vals[index.first * n + index.second]
					<< ", number of comparisions : " << comparisions
					<< ", ratio : " << (comparisions / (double) (m + n))
					<< endl;
		}
		delete[] vals;
	}
